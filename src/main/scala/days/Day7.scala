...
object Day7 {
  ...
  def findAllCombinatorOutcomesWithConcatenator(
      inputList: List[Int],
      testValue: Long
  ): Long = {
    var outcomes: List[Long] = List(inputList(0))
    for (x <- 1 to (inputList.length - 1)) {
      ...
      outcomes.map(intermediateResult => {
        val nextValue = inputList(x)
        outcomes =
          outcomes :+ (intermediateResult * nextValue).toLong :+ (intermediateResult + nextValue).toLong :+ (intermediateResult.toString + nextValue.toString).toLong
      })
      outcomes = outcomes.drop(amountToDrop).filter(_ <= testValue)
    }
    if (outcomes.contains(testValue)) testValue else 0
